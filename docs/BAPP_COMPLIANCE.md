...
| Burp AI rules | Declares `EnhancedCapability.AI_FEATURES` (Burp shows its own consent prompt on load); `api.ai()` is only called from an explicit, opt-in "AI Analysis" button in the Advisory panel; disabled with a message if Burp AI is off; no AI use anywhere else | Ready |
