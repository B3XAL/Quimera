...
public class QuimeraSettings {
    ...
    /**
     * Burp tools Quimera listens to passively by default. Everything the user browses/replays/
     * fuzzes/scans through these tools is analyzed automatically, "pasiva por defecto, escucha
     * todo". EXTENSIONS is included too: in practice a lot of legitimate application traffic
     * (background requests re-issued or relayed by other loaded extensions, SPA API calls, etc.)
     * shows up under this tool source, not just Quimera's own active probes, excluding it means
     * missing real traffic. Any occasional double-processing of Quimera's own probe requests is
     * harmless (same URL just gets re-analyzed in place).
     */
    public static final Set<String> DEFAULT_ENABLED_TOOLS = Set.of(
            ToolType.PROXY.name(), ToolType.REPEATER.name(), ToolType.INTRUDER.name(),
            ToolType.SCANNER.name(), ToolType.TARGET.name(), ToolType.EXTENSIONS.name()
    );
    ...
    /** When true, passive capture only analyzes in-scope traffic. */
    private volatile boolean restrictToScope = true;
     ...
     * traffic, not just when the user explicitly runs "Active header scan". Off by default: it
     * generates real extra requests against the target for every distinct URL, an explicit opt-in.
     * Which individual probes actually run is still governed by activeScan*Probe above. */
    private volatile boolean autoActiveScan = false;
    /** Optional third-party validation of exposed AIza keys. Requires autoActiveScan too; kept
     * separately off by default because the read-only Google calls can consume project quota. */
    private volatile boolean googleApiKeyProbeEnabled = false;
    ...
    // replays the original request. Off by default, unlike every other JWT check above this is
    // NOT passive recognition, it sends real forged-authentication requests at the target,
    // automatically, once per distinct token seen, the instant this is turned on.
    private volatile boolean jwtActiveProbeEnabled      = false;
    ...
    // credential to check whether the server still accepts it. Off by default, same sensitivity
    // class as jwtActiveProbeEnabled, sends real replayed requests at the target automatically.
    private volatile boolean sessionInvalidationProbeEnabled = false;
    ...
    /** Controls only the bounded CORS/TRACE/HSTS battery. Credential forgery and stale-session
     * replay are deliberately separate opt-ins because they have materially different effects. */
    public void setAutoActiveScan(boolean v) {
        autoActiveScan = v;
        persist();
    }
    ...
    public synchronized void reset() {
        ...
        autoActiveScan = false;
        googleApiKeyProbeEnabled = false;
        ...
        jwtActiveProbeEnabled = false;
        sessionInvalidationProbeEnabled = false;
