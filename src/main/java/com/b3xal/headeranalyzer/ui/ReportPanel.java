...
public final class ReportPanel extends JPanel {
    ...
    private JPanel buildSecuritySection() {
        JPanel grid = new JPanel(new CardGridLayout(3, 10, 8));
        ...
        for (String[] spec : SEC_HEADERS) {
            ...
            grid.add(buildCard(hdr, label, desc, ri, retestNote));
        }
    ...
    private JPanel buildDisclosureSection() {
        ...
        if (!techInventory.isEmpty()) {
            JPanel techGrid = new JPanel(new CardGridLayout(3, 10, 8));
            techGrid.setOpaque(false);
            for (TechFinding tf : techInventory) {
                techGrid.add(buildCard(tf.sourceHeader, tf.display(),
                        "Technology fingerprint parsed from " + tf.sourceHeader + ".",
                        new RowInfo(Status.EXPOSED, tf.display(), "Remove/obscure " + tf.sourceHeader + " to reduce fingerprinting surface."),
                        null));
            }
            techGrid.setAlignmentX(LEFT_ALIGNMENT);
            sec.add(techGrid);
            sec.add(vgap(10));
        }

        if (exposed.isEmpty() && techInventory.isEmpty()) {
            ...
            JPanel grid = new JPanel(new CardGridLayout(3, 10, 8));
            grid.setOpaque(false);
            for (String hdr : exposed) {
                String value = exposedValue.get(hdr);
                HeaderFinding finding = exposedFinding.get(hdr);
                int count = exposedCount.get(hdr);
                String seenNote = hostResults.size() > 1
                        ? "Seen on " + count + " of " + hostResults.size() + " request(s) analyzed on this host."
                        : null;
                grid.add(buildCard(hdr, finding.issueName, disclosureSummary(finding),
                        new RowInfo(Status.EXPOSED, trunc(value, 70), finding.evidence), seenNote));
            }
            grid.setAlignmentX(LEFT_ALIGNMENT);
            sec.add(grid);
        }
    ...
    private JPanel buildCookieSection(List<HeaderFinding> cookieFindings) {
        ...
        grid.setOpaque(false);
        for (HeaderFinding f : cookieFindings) {
            JPanel row = new JPanel(new BorderLayout(8, 0));
            row.setOpaque(true);
            row.setBackground(f.severity == Severity.INFORMATION
                    ? (dark ? new Color(24, 35, 54) : new Color(240, 248, 255))
                    : Status.WARNING.cardBg(dark));
            row.setBorder(BorderFactory.createEmptyBorder(6, 10, 6, 10));
            JLabel lbl = new JLabel(f.issueName);
            lbl.setFont(lbl.getFont().deriveFont(Font.BOLD, 11f));
            lbl.setForeground(dark ? new Color(230, 232, 236) : new Color(20, 20, 25));
            row.add(lbl, BorderLayout.CENTER);
            JLabel sevLbl = new JLabel(f.severity.label);
            sevLbl.setOpaque(true);
            sevLbl.setBackground(f.severity.color);
            sevLbl.setForeground(Color.WHITE);
            sevLbl.setFont(sevLbl.getFont().deriveFont(Font.BOLD, 10f));
            sevLbl.setBorder(BorderFactory.createEmptyBorder(2, 6, 2, 6));
            row.add(sevLbl, BorderLayout.EAST);
            grid.add(row);
        }
        sec.add(grid);
        return sec;
    ...
    private RowInfo evaluateSecurityHeader(String hdr, String value) {
        ...
        return switch (hdr) {
            ...
            case "Referrer-Policy" -> {
                if (value == null) yield new RowInfo(Status.MISSING, "Not present", "Recommended: strict-origin-when-cross-origin");
                String lc = value.toLowerCase().trim();
                if (lc.contains("unsafe-url")) yield new RowInfo(Status.MISSING, value, "'unsafe-url' leaks full URL, remove.");
                Set<String> safe = Set.of("no-referrer", "no-referrer-when-downgrade", "origin",
                        "origin-when-cross-origin", "strict-origin", "strict-origin-when-cross-origin");
                yield safe.contains(lc) ? new RowInfo(Status.SECURE, value, "Correctly configured.")
                        : new RowInfo(Status.WARNING, value, "Review value.");
            }
