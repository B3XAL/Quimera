...
public final class QuimeraTab {
    ...
    public QuimeraTab(MontoyaApi api,
        ...
        cookiePanel.setOnRowSelected((result, headerHint, issueNameHint, isRequestHeader) -> {
            cookieDetailPanel.show(result, headerHint, issueNameHint, isRequestHeader);
            reportPanel.show(result);
        });
    ...
    public void onResultAdded(UrlAnalysisResult result) {
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                totalFindings = domainStore.values().stream().mapToInt(DomainData::getTotalFindings).sum();
                updateStats();
            } catch (Exception ex) {
    ...
    private void onSettingsApplied() {
        ...
        pruneOutOfScopeIfNeeded();
    }

    /** "Restrict to scope" only ever gated NEW traffic at capture time (QuimeraHttpHandler,
     * HeaderPassiveScanner), so turning it on mid-session left every already-captured
     * out-of-scope row sitting there with no way to remove it short of a full Clear. Checked
     * every time Settings is applied (cheap no-op scan when the toggle is off or nothing is
     * actually out of scope), scope is per-URL not per-host (Burp scope can be path-scoped), so
     * each result is checked individually, not just its host.
     *
     * Rebuilds everything from scratch rather than surgically removing rows: LoggerPanel's
     * group/affected-table bookkeeping has no public "remove this one result" API, and this is a
     * rare, deliberate settings action, not a hot path. One side effect worth knowing: this also
     * resets RetestTracker's fixed/still-open history for every row, in-scope ones included
     * (RetestTracker only supports clearing everything, not a per-host prune), retest state for
     * anything kept just has to be re-earned with a fresh Retest after this runs. */
    private void pruneOutOfScopeIfNeeded() {
        if (!settings.isRestrictToScope()) return;

        List<UrlAnalysisResult> keep = new ArrayList<>();
        boolean anyOutOfScope = false;
        for (DomainData dd : domainStore.values()) {
            for (UrlAnalysisResult r : dd.getUrlResults().values()) {
                if (api.scope().isInScope(r.url)) keep.add(r);
                else anyOutOfScope = true;
            }
        }
        if (!anyOutOfScope) return;

        clearAll();
        for (UrlAnalysisResult r : keep) onResultAdded(r);
    }
