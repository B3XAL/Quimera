...
/**
 * Settings tab: what Quimera listens to passively (which Burp tools, in-scope only or
 * everything), what the active scan probes do, and which file/content-type skip lists apply.
 */
public final class SettingsPanel extends JPanel {
    ...
    private final JCheckBox restrictScopeChk    = new JCheckBox("Only analyze in-scope traffic (passive listening)");
    ...
    private JPanel buildAutoActiveScanCallout() {
        ...
        callout.add(wrappedLabel("URL seen on intercepted proxy traffic, unattended. Off = purely", descColor));
        callout.add(wrappedLabel("passive. JWT forgery and session-replay probes remain separate", descColor));
        callout.add(wrappedLabel("opt-ins because they carry materially different risk.", descColor));
    ...
    /** Refreshes the auto-active-scan checkbox/badge plus the two checkboxes it's coupled to
     * (JWT active probe, session invalidation probe, see {@link QuimeraSettings#setAutoActiveScan})
