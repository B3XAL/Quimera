...
public class HeaderAnalysisEngine {
    ...
    private static final Set<String> SCRIPT_STYLE_EXTENSIONS = Set.of(
            ".js", ".mjs", ".cjs", ".css"
    );
    ...
    private static boolean isMimeSniffingRelevant(Map<String, String> responseHeaders,
                                                    Map<String, String> requestHeaders,
        ...
        if (destination != null && (destination.equalsIgnoreCase("script")
                || destination.equalsIgnoreCase("style"))) return true;

        String path = extractPath(rawUrl).toLowerCase(Locale.ROOT);
        int dot = path.lastIndexOf('.');
        if (dot >= 0 && SCRIPT_STYLE_EXTENSIONS.contains(path.substring(dot))) return true;
        ...
        return mime.contains("javascript") || mime.equals("text/css");
    }
