        ...
        return List.of(
            ...
            new HeaderRule("Strict-Transport-Security",
                ...
                List.of(
                    ...
                    // includeSubDomains is intentionally not required: without evidence that an
                    // HTTP-capable subdomain exists, its absence is only deployment advice and
                    // can be actively wrong for domains with non-HTTPS legacy subdomains.
                ),
                "https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Strict-Transport-Security"
            ),
            ...
            // Absence and wholly-invalid values fall back to strict-origin-when-cross-origin in
            // modern browsers, so neither is a finding. Effective unsafe policies are parsed in
            // HeaderAnalysisEngine because comma-separated fallback lists require selecting the
            // last recognised token, something a broad per-value regex cannot model correctly.
            new HeaderRule("Referrer-Policy", List.of()),
            ...
            // Missing/invalid COOP resolves to unsafe-none. That can matter to an application
            // deliberately requiring cross-origin isolation, but it is not a generic vulnerability
            // without evidence of such a requirement, so no presence-only rule is emitted.
            ...
            // Missing Permissions-Policy is not a vulnerability. Feature-specific defaults and
            // the browser permission model still apply; without evidence that a page embeds
            // untrusted content while exposing a powerful feature, a generic finding is noise.
            ...
            // Access-Control-Allow-Methods advertising TRACE is not a browser exploit: Fetch
            // forbids scripts from issuing TRACE regardless of CORS permission. The independent
            // active TRACE probe verifies whether the server really accepts and echoes TRACE.
