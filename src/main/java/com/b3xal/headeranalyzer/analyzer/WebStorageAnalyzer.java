...
public final class WebStorageAnalyzer {
    ...
    public static boolean looksLikeOpaqueToken(String value) {
        ...
        // Absolute and relative application routes are not secrets. The latter matters for globals
        // such as window.unlockPassword = "users/unlock-user": the name looks credential-related,
        // but the value is plainly a route. '+' and '=' distinguish conventional base64 material
        // containing '/' from these URL-path-shaped strings.
        if (v.startsWith("/")
                || (v.contains("/") && !v.contains("+") && !v.contains("=")
                    && v.matches("[A-Za-z0-9._~-]+(?:/[A-Za-z0-9._~-]+)+/?"))) return false;
