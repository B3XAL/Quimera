...
<picture>
  ...
  <img src="Quimera.png" alt="Quimera logo" width="480">
</picture>
...
- Advisory panel alongside the raw Request/Response view, the full finding description and
  remediation guidance in place, the same way Burp's own Issue Advisory tab works. An opt-in
  "AI Analysis" button there uses Burp's own AI (`api.ai()`, Settings > AI in Burp Suite Pro) to
  write a client-ready explanation of one finding or a whole URL; it only runs when clicked, and
  does nothing if Burp AI is off.
...
**Full step-by-step pairing walkthrough** (generating the token in Burp, pasting it into the
extension, granting host access, and troubleshooting a failed connection):
[quimera-extension README, "Pairing with Quimera Burp, step by step"](https://github.com/B3XAL/Quimera-extension#pairing-with-quimera-burp-step-by-step).
...
crawled, or actively sweep an entire host/target. JWT forgery and session-invalidation replay are
off by default, but are automatically selected when Auto Active Scan is enabled. Exposed Google
API-key validation is selected by default. All three remain individual switches.
